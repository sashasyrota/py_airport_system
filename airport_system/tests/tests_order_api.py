...
from airport_system.serializers import OrderListSerializer, OrderRetrieveSerializer
...
class AuthenticatedTests(TestCase):
    ...
    def test_create_order_with_tickets(self):
        ...
        result = self.client.post(ORDER_URL,order_data(user=self.user.id, tickets=tickets_data), format="json")
